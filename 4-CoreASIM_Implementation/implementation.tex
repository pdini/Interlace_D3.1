...
At the moment there are several message types part of debit and credit requests. Table \ref{tab:impl-msg-types} is giving an overview which messages are in use for handling transfer operations. The messages are listed in the order of occurrence. Bold messages are operation (debit, credit) agnostic but vary based on their transfer parameters.
...
CreditPreviewReq & first check of credit request & transfer parameters \\[3pt]
\hline
\textbf{ProceedMessage} & answer from server that the CreditPreviewReq has been successful & transfer parameters \\[3pt]
\hline
\textbf{DoNotProceedMessage} & answer from server that the CreditPreviewReq has NOT  been successful & error text, transfer parameters \\[3pt]
\hline
CreditPerformReq & request to actually perform a credit request & transfer parameters \\[3pt]
\hline
\textbf{NoPermitted} & answer from server that the CreditPerformReq has NOT  been successful & error text, transfer parameters \\[3pt]
\hline
\textbf{TransferPerformedSuccessful}\ & answer from server that the CreditPerformReq has been successful and the transfer has been recorded and confirmed & transfer parameters \\[3pt]
\hline
...
\caption{\small\textbf{Message types overview}}
\label{tab:impl-msg-types}
\end{centering}
\vspace{-0.5cm}
\end{table}
...
what is implemented, what is missing, centralized architecture