...
\item Case 1. When, for a pending transaction $t$, the $ConfirmationReq(t)$ is answered by the $debitor$ but too late, an error message informing that the $otp$ expired is sent to the debitor and the creditor and the message is discarded. In this case, the $\ASM{DebitLateAckReqAnswCompletion}$ rule below will delete the pending $t$ together with its $otp$ and update $status(t)$ to $rejected$.
...
\ASM{Send}(OtpExpiredMsg(Debit,amount,creditor),\TO debitor)  \dec \-
