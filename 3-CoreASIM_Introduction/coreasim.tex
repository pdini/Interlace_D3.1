...
saying that on the host system the folder \textit{data} is used and on the guest system a folder \textit{vagrant-data} is mounted which are shared and thus containing the same content. Additionally during provisioning a symbolic link in the home directory is created called project (/home/ubuntu/project) which is linking the mounted root folder /vagrant-data.
...
This line is part of the script \textit{scripts/runDocker.sh}. \$1 here stands for the home directory of the user and \$2 will be replace by script which should be executed inside of the docker container.
