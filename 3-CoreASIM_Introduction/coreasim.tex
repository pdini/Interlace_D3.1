...
For the well-known Integrated Development Environment Eclipse, developers of the ICEF framework have adopted the coreASM plugin for supporting the additional language primitives. So when implementing ASIMs using BSL it is highly advised to that Plug-In. It is working with the new language elements (BSL) and is provided by the ICEF framework directly but it needs to be built and imported \textbf{manually} into Eclipse.

Thus for working with ASIM implementations of INTERLACE you ideally DO NOT download the original coreASM plugin package which is available at the marketplace of Eclipse.
